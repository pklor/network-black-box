...
def con_nect(db_path: Path) -> sqlite3.Connection:
    db_path.parent.mkdir(parents=True, existOK=True)
...
def init_db(db_path: Path, config: BBConfig) -> None:
    conn = con_nect(db_path)
    try: 
...
# the on conflict is when the key column violates the unique constraint and excluded.value refers the the value from the first insert
